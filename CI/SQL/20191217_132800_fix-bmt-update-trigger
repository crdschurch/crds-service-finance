...
/****** Object:  Trigger [dbo].[crds_tr_Write_Distribution_Adjustments]    Script Date: 12/12/2019 2:10:37 PM ******/
SET ANSI_NULLS ON
GO
...
IF (UPDATE(Amount) OR UPDATE(Program_ID) OR UPDATE(Congregation_ID))
  BEGIN
	DECLARE @DonationDistributionId INT = (SELECT TOP(1) Donation_Distribution_ID FROM INSERTED);
	DECLARE @DonationId INT = (SELECT TOP(1) [Donation_ID] FROM [Donation_Distributions] WHERE [Donation_Distribution_ID] = @DonationDistributionId);
	DECLARE @IsExported BIT = (SELECT dbo.[crds_IsDonationExported](@DonationId));

	-- check that date falls within the current fiscal year (this should be broken out into a function in the future)
	...
	-- rule 1: Donation in current year and fiscal year is in 1 to 8 months
	IF ((YEAR(@CurrentDate) = YEAR(@DonationDate)) AND (1 <= MONTH(@DonationDate) AND MONTH(@DonationDate) <= 8)
		AND (1 <= MONTH(@CurrentDate) AND MONTH(@CurrentDate) <= 8))
			SELECT @ValidDate = 1

	-- rule 2: Donation in current year and fiscal year is in 9 to 12 months
	IF ((YEAR(@CurrentDate) = YEAR(@DonationDate)) AND (9 <= MONTH(@DonationDate) AND MONTH(@DonationDate) <= 12)
		AND (9 <= MONTH(@CurrentDate) AND MONTH(@CurrentDate) <= 12))
			SELECT @ValidDate = 1

	-- rule 3: Donation is in previous year but still in current fiscal year
	IF (((YEAR(@CurrentDate) - 1) = YEAR(@DonationDate)) AND (9 <= MONTH(@DonationDate) AND MONTH(@DonationDate) <= 12)
		AND (1 <= MONTH(@CurrentDate) AND MONTH(@CurrentDate) <= 8))
			SELECT @ValidDate = 1

	-- rule 4: Donation is in current fiscal year and next year
	IF (((YEAR(@CurrentDate) + 1) = YEAR(@DonationDate)) AND (1 <= MONTH(@DonationDate) AND MONTH(@DonationDate) <= 8)
		AND (9 <= MONTH(@CurrentDate) AND MONTH(@CurrentDate) <= 12))
			SELECT @ValidDate = 1

	IF (@IsExported = 1 AND @ValidDate = 1)
	  BEGIN
		...
		--Fetch CongregationId from previous distributions if not set
